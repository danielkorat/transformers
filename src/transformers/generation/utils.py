    ...
    def _assisted_decoding(
        ...
        **sample** (depending on `do_sample`), assisted by candidate sequences. Assisted generation is an example of a
        ...
        while self._has_unfinished_sequences(this_peer_finished, synced_gpus, device=input_ids.device):
            ...
            if candidate_input_ids.shape[1] > 930: #and not (some[:,:model_inputs["input_ids"].shape[1]].cpu() == model_inputs["input_ids"].cpu()).all():
                pdb.set_trace()
            ...
            if do_sample and candidate_logits is not None:
                ...
                # pylogging.error(f"SAMPLE {valid_tokens.tolist()=}")
            # Case 2: all other cases (originally from assisted generation) 👉 Compare the tokens selected from the
            # original model logits with the candidate tokens. We can keep the candidate tokens until the first
            # mismatch, or until the max length is reached.
            else:
                if do_sample:
                    # pylogging.error(f"SAMPLE OH NO!!!!!!!!!!")
                ...
                # pylogging.error(f"GREEDY? {valid_tokens.tolist()=}")
        ...
        input_ids = input_ids[:, :last_valid_length]
